...
func TestRegisterServiceDefinition(t *testing.T) {
	...
	assert.Regexp(t, regexp.MustCompile(`app-[0-9]+\.nexentraedge\.cloud`), serviceDefinition.Host)
}
...
func TestRegisterServiceDefinitionWithHost(t *testing.T) {
	repo := NewInMemoryServiceDefinitionRepository()
	serviceDefinitionManager := NewServiceDefinitionManager(repo)
	err := serviceDefinitionManager.RegisterServiceDefinition("test", "test", "test", 8080, "test.nexentraedge.cloud")
	if err != nil {
		t.Errorf("Failed to register service definition: %s", err)
	}
	...
	assert.Equal(t, "test.nexentraedge.cloud", serviceDefinition.Host)
}
